...
interface CompanyPeopleListProps {
  companyNormalized: string;
  expanded: boolean;
}

function CompanyPeopleList({ companyNormalized, expanded }: CompanyPeopleListProps) {
          ...
          {data.people.map((person) => (
            ...
            >
              ...
              <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-hidden text-xs">
                <span className="shrink-0 truncate font-medium text-foreground">{person.fullName}</span>
                {person.position && (
                  <>
                    <span className="shrink-0 text-muted-foreground/40">·</span>
                    <span className="truncate text-muted-foreground">{person.position}</span>
                  </>
                )}
                {person.isStarred && (
...
export function UnmatchedPeopleModal({
  open,
  onOpenChange,
  companies,
}: UnmatchedPeopleModalProps) {
      ...
      <AlertDialogContent className="!left-1/2 !top-1/2 !max-h-[90vh] !-translate-x-1/2 !-translate-y-1/2 max-w-[96vw] overflow-y-auto data-[size=default]:max-w-[96vw] data-[size=default]:sm:max-w-5xl">
        ...
        <AlertDialogHeader className="place-items-start text-left">
          <AlertDialogTitle className="flex items-center gap-2 text-base">
            ...
            Review Unmapped People Companies
          </AlertDialogTitle>
          <AlertDialogDescription className="text-sm">
            Map imported people company names to tracked companies, or ignore those you do not want to manage.
          </AlertDialogDescription>
        </AlertDialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-2.5 py-1 text-amber-300">
              {data?.summary.unmatchedPeopleCount || 0} unmatched people
            </span>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div className="flex flex-wrap items-center gap-2" style={{ alignSelf: "center" }}>
                        <Button
                          variant="ghost"
                          size="icon-sm"
                          title={isExpanded ? "Hide people" : "View people"}
                          onClick={() => {
                            setExpandedCompanies((prev) => ({
                              ...prev,
