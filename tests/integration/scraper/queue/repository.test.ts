...
const sqlite = createSqliteTestHarness("switchy-queue-");
...
function createQueueDatabases() {
  const {
    ...
    path,
  } = sqlite.createDatabase({ busyTimeoutMs: 1, fileName: "queue.db" });
  const { database: secondDatabase } = sqlite.connect(path, { busyTimeoutMs: 1 });
