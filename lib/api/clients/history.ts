...
export const getMatchHistoryDetail = (sessionId: string) => apiRequest(`/api/match-history/${encodeURIComponent(sessionId)}`, { method: "GET", cache: "no-store" }, matchHistoryDetailResponseSchema, "Failed to fetch match history");
...
export const getScrapeHistoryDetail = (sessionId: string) => apiRequest(`/api/scrape-history/${encodeURIComponent(sessionId)}`, { method: "GET", cache: "no-store" }, scrapeHistoryDetailResponseSchema, "Failed to fetch scrape history");
