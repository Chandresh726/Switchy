...
OpenCode's own free-tier models reject sessions created through the HTTP client. Switchy executes those models through the supported `opencode run` automation command in an empty temporary directory and removes the resulting session after completion when possible. OpenCode requires its built-in agent for this path, so free-tier execution does not have the deny-all session isolation used by the HTTP-client path. Models backed by user-owned OpenCode provider connections continue to use isolated sessions with ordered deny-all permissions.
