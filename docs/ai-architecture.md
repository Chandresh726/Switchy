...
OpenCode's own free-tier models reject external integrations and are therefore excluded from Switchy's model catalog. OpenCode remains keyless from Switchy's perspective, but execution requires a provider connection that the user owns and authenticates in OpenCode. Switchy does not fall back to `opencode run`, because that path cannot preserve per-session deny-all isolation for free-tier models.
