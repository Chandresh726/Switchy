  ...
  try {
    ...
    const contentsWithHistory = contents.map((content) => ({
      ...
      history: (historyByContentId.get(content.id) || []).map((h) => ({
        ...
        parentVariantId: h.parentVariantId,
        aiRunId: h.aiRunId,
        source: h.source,
        selectedAt: h.selectedAt?.toISOString() || null,
        copiedAt: h.copiedAt?.toISOString() || null,
        discardedAt: h.discardedAt?.toISOString() || null,
        editDistance: h.editDistance,
        editDistanceRatio: h.editDistanceRatio,
